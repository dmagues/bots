...
bot.dialog('/', [
    ...
    function (session, results) {
        if (results.response) {
            session.endDialog(`Me contaste que tu nombre es **${session.dialogData.nombre}**, tienes **${session.dialogData.edad}** años, son las **${session.dialogData.hora.getHours}** y prefieres **${session.dialogData.preferencia}**`);
        }
        else {
